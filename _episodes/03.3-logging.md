...
~~~
git log -S"git flow support"
~~~
It is often useful to combine this with the `-p` flag. So we can run
~~~
git log -S"git flow support" -p
~~~
If you want to search using a regular expression instead of a string, you can use the -G"<regex>" flag instead.
~~~
git log -G'usage.*git flow support' -p
~~~
